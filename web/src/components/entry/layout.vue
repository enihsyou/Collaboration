<template>
  <div class="main">
    <router-view @placeholderAnimationIn="placeholderAnimationIn"
                 @placeholderAnimationOut="placeholderAnimationOut"></router-view>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      placeholderAnimationOut(e) {
        ...
        let letters = parent.querySelectorAll('.letter');
        letters = [].slice.call(letters, 0).reverse();
        letters.forEach(function (el, i) {
          setTimeout(function () {
            let contains = parent.classList.contains('filled');
            if (contains) return;
